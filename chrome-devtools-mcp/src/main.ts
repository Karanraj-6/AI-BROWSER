...
// The MCP server will not auto-launch Chrome on startup. Chrome (and a
// remote-debugging endpoint) will be started on-demand when tools request a
// browser or when an extension connects via the WebSocket bridge.
...
import WebSocketServerTransport from './tools/WebSocketServerTransport.js';

const transport = new StdioServerTransport();
await server.connect(transport);
logger('Chrome DevTools MCP Server connected');
// Keep the server alive until the stdio transport is closed by a client.
...
try {
  ...
  try {
    // server.connect expects a transport; the McpServer.connect delegates to
    // the underlying Server implementation which may accept multiple transports.
  ...
  logger('Failed to start MCP WebSocket bridge: ' + (e instanceof Error ? e.message : String(e)));
}
logDisclaimers();
